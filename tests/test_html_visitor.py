...
class TestHtmlVisitor(unittest.TestCase):
    ...
    def test_valid_cve_entry_to_html(self):
        ...
        self.assertEqual(td_tags[0].text, 'CVE_2021_123')
        self.assertEqual(td_tags[1].text, '9.0')
        self.assertEqual(td_tags[2].text, 'cvss_vector_here')
        self.assertEqual(td_tags[3].text, 'boto3')
        self.assertEqual(td_tags[4].text, '1.9.91')
        self.assertEqual(td_tags[5].text, 'http://example.com')
        self.assertEqual(td_tags[6].text, 'A fake vulnerability.')
    ...
    def test_full_basic_report(self):
        ...
        old_cve = {'MODULE_SOURCE': 'Pip Dependencies', 'ID': 'CVE_2021_456', 'ModuleName': 'boto3',
                   ...
                   'URL': 'http://example.com', 'is_new': False}
        ...
        self.assertEqual(td_tags[7].text, 'CVE_2021_456')
        self.assertEqual(td_tags[8].text, '8.0')
        self.assertEqual(td_tags[9].text, 'another_cvss_vector_here')
        self.assertEqual(td_tags[10].text, 'boto3')
        self.assertEqual(td_tags[11].text, '1.9.91')
        self.assertEqual(td_tags[12].text, 'http://example.com')
        self.assertEqual(td_tags[13].text, 'Another fake vulnerability.')
