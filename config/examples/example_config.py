...
class Config:
    ...
    """
    NIST_TOKEN = 'YOUR NIST TOKEN'

    """
    Configure any combination of dependency searchers and package parsers below. Note that
    ...
    pattern to parser mappings per searcher configuration. Only searcher configurations appended
    ...
    GitHub configuration example:
     SEARCHER_CONFIGS.append({
         "searcher": GitHubRepoSearcher("github_token_here", "project/repo"),
         "search_pattern": {"package.json": NpmParser(),
                            "*.bzl": BazelParser()}
         "search_path": "example/path" # optional, defaults to root directory
         })
    ...
    GitLab configuration example:
     SEARCHER_CONFIGS.append({
            "searcher": GitLabRepoSearcher("gitlab_token_here", "gitlab_url_here", "project/repo"),
    ...
    Local file configuration example:
     SEARCHER_CONFIGS.append({
         ...
         "search_path": "example/path" # optional, defaults to current working directory
         })
    ...
    SEARCHER_CONFIGS.append({
        "search_uri": GitHubRepoSearcher(GITHUB_TOKEN, 'motional/nuplan-devkit'),
        "search_pattern": {"requirements.txt": PipParser()}
    })
    SEARCHER_CONFIGS.append({
        "searcher": LocalFileSearcher(),
         "search_pattern": {"sources.csv": CsvParser()},
